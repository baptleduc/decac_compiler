...
public class DeclMethod extends AbstractDeclMethod {
    ...
    protected EnvironmentExp verifyMethod(DecacCompiler compiler, AbstractIdentifier superClass, int index)
		...
		methodSuperClass = superClassDefinition.asMethodDefinition("is not a method definition", methodName.getLocation());
		sign2 = methodSuperClass.getSignature();
		type2 = methodSuperClass.getType();

		if ((signature.sameSign(sign2)) && (type1.sameType(type2) || type1.asClassType("not a class Type", methodName.getLocation()).isSubClassOf(type2.asClassType("not a class Type", methodName.getLocation())))){
			//Override Condition
